...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "\n" + _("RPC SSL options: (see the Yescoin Wiki for SSL setup instructions)") + "\n";
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Yescoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Yescoin is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.yescoin.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Yescoin Core is shutting down."));
...
#endif
    // Make sure only a single Yescoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Yescoin Core is probably already running."), strDataDir));
    ...
    if (pwalletMain)
        GenerateYescoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
