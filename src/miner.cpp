...
void static YescoinMiner(CWallet *pwallet)
{
    LogPrintf("YescoinMiner started\n");
    ...
    try {
        while (true) {
            ...
            LogPrintf("Running YescoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("YescoinMiner terminated\n");
        throw;
    }
}
...
void GenerateYescoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YescoinMiner, pwallet));
}
